...
export initialstate, genkrylov, thomas, clusterGreenFunction, clusterGreenFunctionLoop, origiQuadraticTerms!, referQuadraticTerms, CPTcore, seqs
export perGreenFunction, GreenFunctionPath, periodization, singleParticleGreenFunction, spectrum, energysurface, select, statedensity, saveData, loadData
export GPcore, GPintegrand, GrandPotential, VCAs
...
struct Kryvals{T<:AbstractVector, S<:AbstractVector, R<:AbstractVector, N<:AbstractVector, P<:AbstractMatrix}
    ...
    function Kryvals(matrix::AbstractMatrix, initialstates::AbstractVector, m::Int=200)
        ...
        avs, bvs, cvs, norms, projects = Vector{Vector}(undef,n), Vector{Vector}(undef,n), Vector{Vector}(undef,n), Vector{Float64}(undef,n), Matrix{Vector{ComplexF64}}(undef,n,n)
        orth = KrylovKit.ModifiedGramSchmidt()
        for i in 1:n
            krybasis, avs[i], bvs[i], cvs[i] = genkrylov(matrix, initialstates[i], orth, m)
            norms[i] = real(√(initialstates[i]' * initialstates[i]))
            for j in 1:n
                projects[i, j] = KrylovKit.project!(zeros(ComplexF64, m), krybasis, initialstates[j])
            end
        end
...
function initialstatesets(k::EDKind, gs::AbstractVector, target::TargetSpace, table::Table)
    sets₁, sets₂ = [ComplexF64[] for _ in 1:length(table)], [ComplexF64[] for _ in 1:length(table)]
...
"""
    VCA(unitcell::AbstractLattice, cluster::AbstractLattice, hilbert::Hilbert, origiterms::Tuple{Vararg{Term}}, referterms::Tuple{Vararg{Term}}, target::TargetSpace; neighbors::Neighbors, m::Int=200)
...
function VCAs(unitcell::AbstractLattice, cluster::AbstractLattice, hilbert::Hilbert, origiterms::Tuple{Vararg{Term}}, referterms::Tuple{Vararg{Term}}, target::TargetSpace, params::AbstractArray{<:Parameters}; neighbors::Neighbors, m::Int=200)
    k = EDKind(typeof(origiterms))
    table = Table(hilbert, Metric(k, hilbert))
    origibonds = bonds(cluster, neighbors)
    referbonds = filter(bond -> isintracell(bond), origibonds)
...
"""
    origiQuadraticTerms!(om::AbstractMatrix, oops::AbstractVector, oopsseqs::AbstractVector,k::AbstractVector) -> Matrix
...
function singleParticleGreenFunction(vca::VCA, k_path::ReciprocalSpace, ω_range::AbstractRange; timer=vcatimer)
...
function GPintegrand(solver::GFSolver, temp::AbstractMatrix, vmvec::AbstractVector, bz::ReciprocalSpace, ω::Complex)
    intra = 0.0
...
function GrandPotential(vca::VCA, bz::ReciprocalSpace, μ::Real) 
    ...
    trvm = 0.0
    for i in eachindex(bz)
        vmvec[i] = origiQuadraticTerms!(copy(om), oops, oopsseqs, bz[i]) - rm
        trvm += tr(vmvec[i])
    end
    gp = (length(vca.cluster)/(length(vca.unitcell)*length(bz)))*(- quadgk(x -> GPintegrand(vca.solver, temp, vmvec, bz, x*im+μ), 0, Inf)[1]/π + trvm/2)
    return real(gp)
end


function statedensity(vca::VCA, k_path::ReciprocalSpace, ω_range::AbstractRange, μ::Real; η::Real=0.05)
    A = singleparticlespectrum(vca, k_path, ω_range, μ; η = η)
    S = sum(A, dims=2)
    return S
end

function energysurface(vca::VCA, reciprocalspace::ReciprocalSpace, ω_range::AbstractRange, a::Real)
    oops, rops = filter(op -> length(op) == 2, collect(expand(vca.origigenerator))), filter(op -> length(op) == 2, collect(expand(vca.refergenerator)))
    oopsseqs = seqs(oops, vca.origigenerator.table)
    rm = referQuadraticTerms(rops, vca.refergenerator.table)
    om = zeros(ComplexF64, length(vca.refergenerator.table), length(vca.refergenerator.table))
    ...
    for ω in ω_range
        cgf = clusterGreenFunction(vca.solver, ω)
